...
def most_qualifier1_played():
    df1 = data.loc[data['MatchNumber'] == 'Qualifier 1'].Team1.value_counts()
    df1_dict = df1.to_dict()

    df2 = data.loc[data['MatchNumber'] == 'Qualifier 1'].Team2.value_counts()
    df2_dict = df2.to_dict()

    df1 = pd.DataFrame.from_dict(df1_dict.items())
    df1.columns = ['TEAM', 'TOTAL QUALIFIERS 1 PLAYED']

    df2 = pd.DataFrame.from_dict(df2_dict.items())
    df2.columns = ['TEAM', 'TOTAL QUALIFIERS 1 PLAYED']

    df = df1.append(df2)

    agg_functions = {'TOTAL QUALIFIERS 1 PLAYED': 'sum'}
    df = df.groupby(df['TEAM']).aggregate(agg_functions)
    df.reset_index(inplace=True)
    df = df.sort_values(by=['TOTAL QUALIFIERS 1 PLAYED'], ascending=False)
