...
def bat(team):
    ...
    SVM = svm.SVC(kernel = 'rbf',probability=True) # sum(x*xi)
    ...
    final_bat_predict = SVM.predict_proba(X_bat)
...
def bowl(team):
    ...
    final_bowl_predict = SVM.predict_proba(X_bowl)
...
def all_round(team):
    ...
    final_all_round_predict = SVM.predict_proba(X_all_round)
