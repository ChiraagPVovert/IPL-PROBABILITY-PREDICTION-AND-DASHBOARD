...
def initialize_data():
    ...
    batsman_df = batsman_and_wk_df[['INNINGS_PLAYED_BATTING','RUNS','BATTING_AVG','BATTING_STRIKE_RATE','IN_LATEST_SQUAD']]
    # batsman_df.shape

    batsman_df[['INNINGS_PLAYED_BATTING','RUNS','BATTING_AVG','BATTING_STRIKE_RATE','IN_LATEST_SQUAD']] = batsman_df[['INNINGS_PLAYED_BATTING','RUNS','BATTING_AVG','BATTING_STRIKE_RATE','IN_LATEST_SQUAD']].astype(float)
    # batsman_df.info()

    # print(batsman_df.isnull().sum())

    data = batsman_df[['INNINGS_PLAYED_BATTING','RUNS','BATTING_AVG','BATTING_STRIKE_RATE','IN_LATEST_SQUAD']]
    # y = batsman_and_wk_df[['RECENTLY_PLAYING_IN_ODI']]
    ...
    x_train = df_train[['INNINGS_PLAYED_BATTING','RUNS','BATTING_AVG','BATTING_STRIKE_RATE']]
    x_test = df_test[['INNINGS_PLAYED_BATTING','RUNS','BATTING_AVG','BATTING_STRIKE_RATE']]
...
def model_predictions(team):
    ...
    #batsman_and_wk_df = batsman_and_wk_df[['NO_OF_MATCHES_PLAYED', 'RUNS', 'BATTING_AVG', 'BATTING_STRIKE_RATE']].copy()
    ...
    X_bat = X_bat.transpose()

    prediction, prob = predict(params, X_bat)

    #return np.transpose(prob)

    final_bat_predict_df = pd.DataFrame(np.transpose(prob),columns=['PROBABILITY_OF_PLAYING'])
...
#prob = model_predictions('Gujurat Titans')
