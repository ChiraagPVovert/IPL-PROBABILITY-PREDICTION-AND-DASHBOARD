...
def initialize_data():
    ...
    bowler_df = bowler_df[['NO_OF_MATCHES_PLAYED','INNINGS_BOWLED','WICKETS','BOWLING_AVERAGE','ECONOMY','BOWLING_STRIKE_RATE','IN_LATEST_SQUAD']]
    # bowler_df.shape

    bowler_df[['NO_OF_MATCHES_PLAYED','INNINGS_BOWLED','WICKETS','BOWLING_AVERAGE','ECONOMY','BOWLING_STRIKE_RATE','IN_LATEST_SQUAD']] = bowler_df[['NO_OF_MATCHES_PLAYED','INNINGS_BOWLED','WICKETS','BOWLING_AVERAGE','ECONOMY','BOWLING_STRIKE_RATE','IN_LATEST_SQUAD']].astype(float)
    # bowler_df.info()

    # print(bowler_df.isnull().sum())

    data = bowler_df[['INNINGS_BOWLED','WICKETS','BOWLING_AVERAGE','ECONOMY','BOWLING_STRIKE_RATE','IN_LATEST_SQUAD']]
    # y = bowler_df[['RECENTLY_PLAYING_IN_ODI']]
    ...
    x_train = df_train[['INNINGS_BOWLED','WICKETS','BOWLING_AVERAGE','ECONOMY','BOWLING_STRIKE_RATE']]
    x_test = df_test[['INNINGS_BOWLED','WICKETS','BOWLING_AVERAGE','ECONOMY','BOWLING_STRIKE_RATE']]
...
def model_predictions(team):
    ...
    params = {'W1': lr_model['W1'], 'b1': lr_model['b1'], 'W2': lr_model['W2'], 'b2': lr_model['b2'],
              'W3': lr_model['W3'],
              'b3': lr_model['b3'], 'W4': lr_model['W4'], 'b4': lr_model['b4']}
    ...
    bowler_df = df[df['TEAM'] == team]
    bowler_df = bowler_df[bowler_df['IN_LATEST_SQUAD'].isin(in_latest_squad)]
    bowler_df = bowler_df[bowler_df['ROLE'].isin(bowl_options)]
    ...
    #bowler_df = bowler_df[['INNINGS_BOWLED','WICKETS','BOWLING_AVERAGE','ECONOMY','BOWLING_STRIKE_RATE']].copy()
    ...
    final_bowl_predict_df = pd.DataFrame(np.transpose(prob),columns=['PROBABILITY_OF_PLAYING'])
...
#prob = model_predictions('Chennai Super Kings')
